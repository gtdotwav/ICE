...
export class WebhookTriggers {
  ...
  /**
   * Dispara quando automação de IA é solicitada
   */
  static async onAIAutomationRequested(requestData: any, userId: string): Promise<void> {
    await webhookService.triggerWebhook(
      "ai_automation_requested",
      {
        requestId: requestData.request_id,
        automationType: requestData.automation_details.type,
        prompt: requestData.automation_details.prompt,
        context: requestData.automation_details.context,
        requestedAt: requestData.timestamp,
        callbackUrl: requestData.callback_config.return_url,
      },
      userId,
    )
  }
  ...
  static async onAIAutomationCompleted(automationData: any, userId: string): Promise<void> {
    await webhookService.triggerWebhook(
      ...
      {
        requestId: automationData.request_id,
        ...
        completedAt: new Date().toISOString(),
        processingTime: automationData.result?.metadata?.processing_time,
      },
      userId,
    )
  }
