    ...
    fields: [
      {
        ...
        label: "Tipo de Copy",
        type: "select",
        options: ["headline", "landing_page", "ad_copy", "email_subject"],
      },
    ...
    fields: [
      {
        ...
        label: "Tipo de Email",
        type: "select",
        options: ["marketing", "transactional", "newsletter"],
      },
      { key: "targetAudience", label: "Público-alvo", type: "text" },
...
export function AIAutomationForm({ type, onClose }: AIAutomationFormProps) {
    ...
    try {
      ...
      const response = await fetch("/api/ai-automation/request", {
        ...
        body: JSON.stringify({
          ...
          userEmail: "user@example.com", // Em produção, obter do contexto de auth
        }),
      })
      ...
      if (response.ok) {
        toast({
          ...
          description: `Sua automação de ${config.title} foi enviada. Tempo estimado: ${config.estimatedTime}`,
        })
  ...
  return (
    ...
    >
      <Card
        className={`glass-card border-2 ${config.borderColor} shadow-2xl bg-gradient-to-br ${config.gradientFrom} ${config.gradientTo}`}
      >
        <CardHeader className="pb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Badge
                variant="secondary"
                className="bg-purple-500/20 text-purple-400 border border-purple-500/30 px-3 py-1"
              >
                      ...
                      >
                        ...
                        {field.type === "select" && field.options ? (
                          ...
                          <Input
                            ...
                            max={field.max}
                            value={context[field.key] || ""}
                            onChange={(e) => handleContextChange(field.key, Number.parseInt(e.target.value))}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="glass-card p-4 border border-primary/20">
                      ...
                      <p className="text-sm text-muted-foreground bg-background/50 p-3 rounded-lg border">{prompt}</p>
                    </div>
