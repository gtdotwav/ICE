  ...
  const features = [
    {
      ...
      icon: <BrainCircuit className="h-12 w-12 text-ai-purple" />,
    ...
    },
    {
      ...
      icon: <BarChartBig className="h-10 w-10 text-ai-cyan" />,
    ...
    },
    {
      ...
      icon: <Bot className="h-10 w-10 text-ai-green" />,
      className: "md:col-span-1 md:row-span-1",
    },
    {
      id: "nlp-copywriting",
      ...
      icon: <FileText className="h-8 w-8 text-ice-quantum-400" />,
      className: "md:col-span-1 md:row-span-1",
    },
    {
      ...
      icon: <Target className="h-8 w-8 text-ice-quantum-400" />,
      className: "md:col-span-1 md:row-span-1",
    },
  ]
  ...
  return (
    <section className="py-20 md:py-32 px-4 bg-ice-quantum-900">
      <div className="container mx-auto">
        ...
        {/* O layout em "bento grid" é ideal para exibir features de forma visualmente interessante. */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {features.map((feature, index) => (
            <GlassCard key={feature.id} custom={index + 1} className={`group flex flex-col ${feature.className}`}>
              <div className="absolute inset-0 bg-gradient-to-br from-ice-quantum-700/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <div className="relative z-10 flex flex-col h-full">
                <div className="mb-4">{feature.icon}</div>
                <h3 className="text-xl md:text-2xl font-bold font-display text-white">{feature.title}</h3>
                <p className="mt-2 text-ice-quantum-300 flex-grow">{feature.description}</p>
                {feature.className.includes("large") && (
                  <Button variant="link" className="mt-6 p-0 text-ai-cyan self-start">
